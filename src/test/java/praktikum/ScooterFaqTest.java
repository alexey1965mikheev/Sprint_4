...
public class ScooterFaqTest {
    ...
    public ScooterFaqTest(int index, String answer) {
        this.index = index;
        this.answer = answer;
    }
    ...
    public static void closeCookies() {
        new MainPage(driverRule.getDriver()).open().acceptCookies();
    }

    @Parameterized.Parameters
    public static Object[][] answerTestData() {
        return new Object[][] {
                {0, "Сутки — 400 рублей. Оплата курьеру — наличными или картой."},
                {1, "Пока что у нас так: один заказ — один самокат. Если хотите покататься с друзьями, " +
                        "можете просто сделать несколько заказов — один за другим."},
                {2, "Допустим, вы оформляете заказ на 8 мая. Мы привозим самокат 8 " +
    ...
    @Test
    public void accordingMenu() {
        String actualAnswer = new MainPage(driverRule.getDriver())
                .scrollToQuestionList()
                .waitForQuestion(index)
